...
import { getClothingStudioJob } from "@/lib/clothing-studio-jobs"
...
export async function GET(
  ...
  try {
    ...
    const job = getClothingStudioJob(jobId, userId)
    if (!job) {
    ...
    console.error("[api/clothing-studio/jobs/:jobId] failed:", error)
